...
urlpatterns = [
    ...
    url(r'^vote-question/(?P<slug>[\w\-]+)/(?P<vote_type>[\w\-]+)/$', qa_views.vote_question, name='vote_question'),
    url(r'^vote-answer/(?P<slug>[\w\-]+)/(?P<pk>\d+)/(?P<vote_type>[\w\-]+)/$', qa_views.vote_answer, name='vote_answer'),
